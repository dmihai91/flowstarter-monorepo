...
describe('LLM Utils - File References', () => {
  ...
  describe('createReferencedFilesContext', () => {
    ...
    it('should create context for single file', () => {
      ...
      expect(result).toContain('<FlowstarterArtifact id="referenced-files"');
    ...
    it('should use FlowstarterAction format', () => {
      const result = createReferencedFilesContext(['README.md'], mockFiles);
      expect(result).toContain('<FlowstarterAction type="file"');
      expect(result).toContain('filePath="README.md"');
      expect(result).toContain('</FlowstarterAction>');
    });
...
Can you fix it to properly import from @app/utils.ts?`;
      ...
      expect(result.referencedFilesContext).toContain('<FlowstarterArtifact');
      expect(result.referencedFilesContext).toContain('<FlowstarterAction type="file"');
      expect(result.referencedFilesContext.match(/<FlowstarterAction/g)?.length).toBe(2);
    });
  });
});
...
describe('LLM Utils - Message Processing', () => {
  ...
  describe('simplifyFlowstarterActions', () => {
    it('should simplify FlowstarterAction tags with type="file"', () => {
      const input =
        '<FlowstarterAction type="file" filePath="/src/index.ts">console.log("hello");\nconsole.log("world");</FlowstarterAction>';
      const result = simplifyFlowstarterActions(input);
      expect(result).toContain('...');
      expect(result).not.toContain('console.log("hello")');
    });

    it('should preserve non-file action types', () => {
      const input = '<FlowstarterAction type="shell">npm install</FlowstarterAction>';
      const result = simplifyFlowstarterActions(input);
      expect(result).toBe(input);
    });

    it('should handle multiple file actions', () => {
      const input = `<FlowstarterAction type="file" filePath="/src/a.ts">content a</FlowstarterAction>
<FlowstarterAction type="file" filePath="/src/b.ts">content b</FlowstarterAction>`;
      const result = simplifyFlowstarterActions(input);
    ...
    it('should handle mixed action types', () => {
      const input = `<FlowstarterAction type="file" filePath="/src/index.ts">file content</FlowstarterAction>
<FlowstarterAction type="shell">npm test</FlowstarterAction>`;
      const result = simplifyFlowstarterActions(input);
    ...
    it('should handle multiline file content', () => {
      const input = `<FlowstarterAction type="file" filePath="/src/index.ts">
function hello() {
  console.log("world");
}
</FlowstarterAction>`;
    ...
    it('should return original string if no file actions found', () => {
      const input = 'No flowstarter actions here';
      const result = simplifyFlowstarterActions(input);
      expect(result).toBe(input);
    });

    it('should preserve attributes in opening tag', () => {
      const input = '<FlowstarterAction type="file" filePath="/test.ts" otherAttr="value">content</FlowstarterAction>';
      const result = simplifyFlowstarterActions(input);
  ...
  describe('createFilesContext', () => {
    ...
    it('should create context for all non-ignored files', () => {
      ...
      expect(result).toContain('<FlowstarterArtifact id="code-content"');
      expect(result).toContain('src/index.ts');
      expect(result).toContain('export const hello');
    });
    ...
    it('should wrap content in FlowstarterArtifact', () => {
      const result = createFilesContext(mockFiles);
      expect(result).toMatch(/^<FlowstarterArtifact/);
      expect(result).toMatch(/<\/FlowstarterArtifact>$/);
    });

    it('should use FlowstarterAction for each file', () => {
      const result = createFilesContext(mockFiles);
      const actionCount = (result.match(/<FlowstarterAction type="file"/g) || []).length;
