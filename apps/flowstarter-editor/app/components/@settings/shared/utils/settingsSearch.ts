...
import type { SettingValue } from '~/types/common';
...
const RECENT_SETTINGS_KEY = 'Flowstarter_recent_settings';
...
const loadRecentSettings = () => {
  try {
    ...
    if (stored) {
      const parsed = JSON.parse(stored) as Array<RecentSetting & { lastAccessed: string }>;
      recentSettings = parsed.map((item) => ({
        ...item,
...
export const updateSettingValue = (settingId: string, newValue: SettingValue) => {
  const setting = settingsRegistry.find((s) => s.id === settingId);
