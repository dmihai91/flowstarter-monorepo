...
const NotificationsTab = () => {
  ...
  const filterOptions: { id: FilterType; label: string; icon: string; color: string }[] = [
    ...
    { id: 'info', label: 'Information', icon: 'i-ph:info', color: '#4D5DD9' },
    { id: 'provider', label: 'Providers', icon: 'i-ph:robot', color: '#10b981' },
    { id: 'network', label: 'Network', icon: 'i-ph:wifi-high', color: '#4D5DD9' },
  ];
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content
              className="min-w-[200px] bg-white dark:bg-gray-800/50 rounded-lg shadow-lg py-1 z-[250] animate-in fade-in-0 zoom-in-95 border border-[#E5E5E5] dark:border-[#999999]"
              sideOffset={5}
              align="start"
              side="bottom"
            >
      ...
      <div className="flex flex-col gap-4">
        {filteredLogs.length === 0 ? (
          ...
          filteredLogs.map((log) => {
            const detailsType = typeof log.details?.type === 'string' ? log.details.type : undefined;
            const style = getNotificationStyle(log.level, detailsType);
