...
export const ChatImpl = memo(
  ({ description, initialMessages, storeMessageHistory, importChat, exportChat }: ChatProps) => {
    ...
    const {
      ...
      setImageDataList,
      clearInputState,
    } = useChatInput({ input, chatStarted, handleInputChange });

    // Error handling
    const { llmErrorAlert, handleError, clearLlmErrorAlert } = useChatErrorHandler({
    ...
    const { sendMessage, abort } = useChatSend({
      ...
      provider,
      chatStarted,
      _setChatStarted: setChatStarted,
      setFakeLoading,
      autoSelectTemplate,
      uploadedFiles,
      imageDataList,
    ...
    useEffect(() => {
      ...
      if (prompt) {
        setSearchParams({});
        runAnimation();
        append({ role: 'user', content: formatMessageText(prompt, model, provider.name) });
      }
