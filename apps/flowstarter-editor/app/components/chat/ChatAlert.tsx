...
const ERROR_TYPE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  network: { label: 'Network Error', color: 'bg-[var(--flowstarter-accent-purple)]' },
  permission: { label: 'Permission Error', color: 'bg-[var(--flowstarter-accent-purple)]' },
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="rounded-lg border border-flowstarter-elements-borderColor bg-flowstarter-elements-background-depth-2 p-4 mb-2"
      >
        <div className="flex items-start">
          ...
          >
            <div className="i-ph:warning-duotone text-xl text-flowstarter-elements-button-danger-text"></div>
          </motion.div>
          ...
          <div className="ml-3 flex-1">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                <motion.h3
                  ...
                  className="text-sm font-medium text-flowstarter-elements-textPrimary"
                >
                  {title}
                </motion.h3>
            ...
            {timestamp && (
              <div className="text-xs text-flowstarter-elements-textSecondary mb-2">
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 0.2 }}
              className="mt-2 text-sm text-flowstarter-elements-textSecondary"
            >
              <p>{message}</p>

              {description && (
                <div className="text-xs text-flowstarter-elements-textSecondary p-2 bg-flowstarter-elements-background-depth-3 rounded mt-3 mb-2">
              ...
              {command && (
                <div className="mt-2 mb-2 flex items-center gap-2">
                  <span className="text-xs font-medium text-flowstarter-elements-textSecondary">Command:</span>
                  <code className="text-xs bg-flowstarter-elements-background-depth-3 px-2 py-1 rounded font-mono flex-1">
                    {command}
                  </code>
                  <button
              ...
              {extractedFiles.length > 0 && (
                <div className="mt-3 mb-2">
                  <div className="text-xs font-medium text-flowstarter-elements-textPrimary mb-1">Affected Files:</div>
                  <div className="flex flex-wrap gap-1">
                    {extractedFiles.slice(0, 3).map((file, index) => (
                      <span
                        key={index}
                        className="text-xs bg-flowstarter-elements-background-depth-3 px-2 py-1 rounded font-mono text-flowstarter-elements-textSecondary"
                      >
                        {file.path}
                        {file.line && (
                          <span className="text-flowstarter-elements-button-danger-text">
                            :{file.line}
                    ...
                    {extractedFiles.length > 3 && (
                      <span className="text-xs text-flowstarter-elements-textSecondary px-2 py-1">
              ...
              {(stackTrace || (content && content.length > 200)) && (
                <div className="mt-3">
                  <button
                    ...
                    className="text-xs font-medium text-flowstarter-elements-textPrimary hover:text-accent-500 flex items-center gap-1"
                  >
                    <div
                  ...
                  {isExpanded && (
                    ...
                    >
                      {stackTrace && (
                        <div className="mb-3">
                          <div className="text-xs font-medium text-flowstarter-elements-textPrimary mb-1">
                            Stack Trace:
                          </div>
                          <pre className="text-xs bg-flowstarter-elements-background-depth-3 p-2 rounded overflow-x-auto max-h-40 overflow-y-auto font-mono text-flowstarter-elements-textSecondary whitespace-pre-wrap">
                            {stackTrace}
                          </pre>
                        </div>
                      )}
                      ...
                      {content && (
                        <div>
                          ...
                          <pre className="text-xs bg-flowstarter-elements-background-depth-3 p-2 rounded overflow-x-auto max-h-60 overflow-y-auto font-mono text-flowstarter-elements-textSecondary whitespace-pre-wrap">
                            {content}
                          </pre>
                        </div>
                      )}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={classNames(
                    ...
                    'focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-flowstarter-elements-button-danger-background',
                    'text-flowstarter-elements-button-primary-text',
                    'flex items-center gap-1.5',
                  )}
                >
                  <div className="i-ph:chat-circle-duotone text-flowstarter-elements-textPrimary"></div>
                  Fix
                </button>
                <button
                  onClick={clearAlert}
                  className={classNames(
                    'px-3 py-1.5 rounded-md text-sm font-medium',
                    'bg-flowstarter-elements-button-secondary-background',
                    'hover:bg-flowstarter-elements-button-secondary-backgroundHover',
                    'focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-flowstarter-elements-button-secondary-background',
                    'text-flowstarter-elements-button-secondary-text',
                  )}
                >
                  Dismiss
                </button>
