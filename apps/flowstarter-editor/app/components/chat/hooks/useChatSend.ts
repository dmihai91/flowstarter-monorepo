...
import { createMessageParts, filesToAttachments, formatMessageText } from '~/components/chat/utils/messageHelpers';
...
export interface UseChatSendOptions {
  ...
  chatStarted: boolean;
  _setChatStarted: (started: boolean) => void;
            ...
            if (temResp) {
              ...
              setMessages([
                ...
                { id: `2-${Date.now()}`, role: 'assistant', content: assistantMessage },
                {
                  id: `3-${Date.now()}`,
                  role: 'user',
                  content: formatMessageText(userMessage, model, provider.name),
                  annotations: ['hidden'],
                },
              ]);
              reload(
