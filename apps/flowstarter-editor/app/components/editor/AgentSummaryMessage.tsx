  ...
  return (
    <div
      className={`rounded-lg border text-xs font-mono overflow-hidden ${
        hasErrors ? 'border-[#EF4444]/30 bg-[#EF4444]/5' : 'border-white/8 bg-white/4'
      }`}
    >
      {/* Header */}
      <div
      ...
      >
        ...
        >
          {hasErrors
            ? `${errors.length} error${errors.length !== 1 ? 's' : ''} during generation`
            : 'Generation complete'}
        </span>
        {doneEvent && (
          ...
          {errors.slice(0, 3).map((e, i) => (
            <p key={i} className="text-[11px] text-[#EF4444]/80 break-words">
              {e.type === 'error'
                ? (e as any).message
                : e.type === 'sandbox_exit'
                  ? `Process exited with code ${(e as any).code}`
                  : (e as any).line}
            </p>
          ))}
          {errors.length > 3 && <p className="text-[10px] text-[#EF4444]/50">+{errors.length - 3} more</p>}
        </div>
      )}
