...
function matchesFilter(event: ActivityEvent, filter: FilterType): boolean {
  ...
  if (filter === 'actions') {
    return event.type === 'tool_call' || event.type === 'fix';
  }

  if (filter === 'errors') {
    return event.type === 'error' || event.type === 'fix';
  }
      ...
      >
        ...
        {isActive && (
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            />
            <span style={{ fontSize: '11px', fontWeight: 500, color: c.filterActiveText }}>Building...</span>
          </div>
        )}
      </div>
  ...
  return (
    <div
      style={{
        ...
        borderLeft: `3px solid ${c.thinkBorder}`,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {truncated && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
              ...
              />
              <span style={{ fontSize: '10px', color: c.thinkText }}>{expanded ? 'show less' : 'show more'}</span>
            </div>
          )}
        </div>
...
function ToolCallRow({ event, c }: { event: ToolCallEvent; c: Record<string, string> }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <span style={{ fontSize: '12px', color: cfg.color, fontWeight: 500, flexShrink: 0 }}>{cfg.label}</span>
      <span
        style={{
          fontSize: '12px',
  ...
  return (
    <div
      style={{
        ...
        borderLeft: `3px solid ${c.errorBorder}`,
      }}
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
        <AlertTriangle size={14} style={{ color: c.errorBorder, flexShrink: 0, marginTop: '2px' }} />
        <p style={{ fontSize: '12px', color: c.errorBorder, margin: 0, flex: 1, lineHeight: 1.5 }}>{event.message}</p>
        <span style={{ fontSize: '10px', color: c.textTer, flexShrink: 0, whiteSpace: 'nowrap' }}>
          {relativeTime(event.timestamp)}
        </span>
      </div>
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '8px',
        background: c.fixBg,
        borderLeft: `3px solid ${c.fixBorder}`,
      }}
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: '12px', fontFamily: 'monospace', color: c.fixBorder, margin: 0 }}>{event.action}</p>
        </div>
        <span style={{ fontSize: '10px', color: c.textTer, flexShrink: 0, whiteSpace: 'nowrap' }}>
          {relativeTime(event.timestamp)}
        </span>
