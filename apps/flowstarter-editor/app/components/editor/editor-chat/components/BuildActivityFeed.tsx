...
export function BuildActivityFeed({
  ...
  const fileCount = events.filter(
    (e) => e.type === 'tool_call' && 'action' in e && (e.action === 'create' || e.action === 'edit'),
  ).length;
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: `1px solid ${borderCol}` }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold" style={{ color: colour, letterSpacing: '0.01em' }}>
            {label}
          </span>
        </div>
      ...
      <div style={{ height: '2px', background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }}>
        <motion.div
          style={{ height: '100%', background: `linear-gradient(90deg, ${colour}70, ${colour}dd)` }}
          initial={{ width: 0 }}
          animate={{ width: `${pct}%` }}
      ...
      <div ref={scrollRef} style={{ maxHeight: '200px', overflowY: 'auto', padding: '8px 0' }}>
        {hidden > 0 && (
          <button
            ...
            className="w-full text-center py-1 text-xs hover:opacity-70 transition-opacity"
            style={{ color: muted }}
          >
            ↑ {hidden} earlier
          </button>
        ...
        <AnimatePresence initial={false}>
          {visible.map((ev, i) => {
            ...
            if (ev.type === 'thinking' && 'text' in ev && ev.text) {
              return (
                <motion.div
                  key={ev.id}
                ...
                >
                  <span className="text-xs mt-0.5 shrink-0" style={{ color: muted }}>
                    ✦
                  </span>
                  <span className="text-xs leading-relaxed">
                    {isLast && isActive ? (
                      <TypewriterText text={ev.text} isDark={isDark} />
                    ) : (
                      <span style={{ color: muted }}>{ev.text}</span>
                    )}
                  </span>
                </motion.div>
              );
            ...
            if (ev.type === 'tool_call' && 'path' in ev && (ev.action === 'create' || ev.action === 'edit')) {
              ...
              return (
                ...
                >
                  <span className="text-xs shrink-0 w-4 text-center" style={{ color: col }}>
                    {icon}
                  </span>
            ...
            if (ev.type === 'fix' && 'reasoning' in ev) {
              return (
                <motion.div
                  key={ev.id}
                  initial={{ opacity: 0, x: -4 }}
                ...
                >
                  <span className="text-xs shrink-0" style={{ color: colour }}>
                    →
                  </span>
                  <span className="text-xs" style={{ color: body }}>
                    {'reasoning' in ev ? ev.reasoning : ''}
                  </span>
                </motion.div>
              );
            }
        ...
        {isActive && currentStep && (
          ...
          >
            ...
            <span className="text-xs font-mono" style={{ color: isDark ? '#a5b4fc' : '#4338ca' }}>
              {currentStep}
            </span>
          </motion.div>
        )}
