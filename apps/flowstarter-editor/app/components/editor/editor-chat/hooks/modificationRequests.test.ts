...
function getErrorMessage(error: string): string {
  if (error.includes('API key') || error.includes('ANTHROPIC')) {
    return (
      '��ᴩ� The AI modification feature requires an Anthropic API key to be configured.\n\n' +
...
describe('formatUserMessage', () => {
  ...
  it('adds image indicator for single image', () => {
    ...
    expect(message).toContain('���� Attached: 1 image');
  });
...
describe('modification request flow states', () => {
  it('shows processing message immediately', () => {
    ...
    const processingMessage = hasImages
      ? '���� Processing your images and applying changes...'
  ...
  it('shows success message with response', () => {
    ...
    const successMessage = response || 'ԣ� Changes applied successfully! Refresh the preview to see the updates.';
