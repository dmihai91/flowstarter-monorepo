          ...
          <div className="p-6 bg-white dark:bg-gray-950 relative z-10 max-w-2xl">
              ...
              <div className="flex-1">
                ...
                {showStats && (
                  <div className="flex gap-3 mb-3 text-xs">
                    ...
                    <span className="text-flowstarter-elements-textSecondary">Total: {totalCount}</span>
                  </div>
                )}
                ...
                {failedFilesList.length > 0 && (
                  <div className="mt-4 mb-4">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-sm font-medium text-flowstarter-elements-textPrimary">
                        Failed files ({failedFilesList.length}):
                      </p>
                      {failedFilesList.length > 5 && (
                        <button
                          onClick={() => setIsExpanded(!isExpanded)}
                          className="text-xs text-flowstarter-elements-textSecondary hover:text-flowstarter-elements-textPrimary flex items-center gap-1"
                        >
                          <div
                            className={classNames(
                    ...
                    <div
                      className={classNames(
                        'rounded border border-flowstarter-elements-borderColor bg-flowstarter-elements-background-depth-1 p-2',
                          ...
                          <div key={errorType} className="mb-3 last:mb-0">
                            ...
                            <ul className="space-y-1.5">
                              {files.map((file, index) => (
                                <li
                                  key={index}
                                  className="text-xs bg-flowstarter-elements-background-depth-2 rounded p-2 flex items-start gap-2"
                                >
                                  {hasExclusionFeature && (
                                    <input
                                      type="checkbox"
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="font-mono text-flowstarter-elements-textPrimary break-all">
                                      {file.path}
                                    </div>
                                    <div className="text-flowstarter-elements-textSecondary mt-0.5">{file.error}</div>
                                  </div>
                                </li>
                              ))}
                            </ul>
                    ...
                    {hasExclusionFeature && selectedForExclusion.size > 0 && (
                      <div className="mt-2 text-xs text-flowstarter-elements-textSecondary">
            ...
            <div className="flex justify-end gap-2 mt-6 flex-wrap">
              {hasAIIntegration && failedFilesList.length > 0 && (
                <Button
                  onClick={handleAskAI}
                  className="bg-flowstarter-elements-button-primary-background text-flowstarter-elements-button-primary-text hover:bg-flowstarter-elements-button-primary-backgroundHover flex items-center gap-1.5"
                >
              ...
              {hasExclusionFeature && selectedForExclusion.size > 0 && (
                <Button
                  ...
                  className="border-flowstarter-elements-borderColor text-flowstarter-elements-textPrimary hover:bg-flowstarter-elements-item-backgroundActive"
                >
