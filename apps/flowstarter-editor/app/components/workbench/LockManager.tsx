...
export function LockManager() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Controls */}
      <div className="flex items-center gap-1 px-2 py-1 border-b border-flowstarter-elements-borderColor">
        {/* Search Input */}
        ...
        {/* Filter Select */}
        <select
          className="text-xs px-1 py-0.5 h-6 bg-flowstarter-elements-background-depth-2 text-flowstarter-elements-textPrimary rounded border border-flowstarter-elements-borderColor focus:outline-none"
          value={filter}
      ...
      <div className="flex items-center justify-between px-2 py-1 text-xs text-flowstarter-elements-textSecondary">
        <div>
          <Checkbox
            checked={selectAllCheckedState}
            onCheckedChange={handleSelectAll}
            className="w-3 h-3 rounded border-flowstarter-elements-borderColor mr-2"
      ...
      <div className="flex-1 overflow-auto modern-scrollbar px-1 py-1">
        {filteredAndSortedItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-flowstarter-elements-textTertiary text-xs gap-2">
            <span className="i-ph:lock-open-duotone text-lg opacity-50 text-flowstarter-elements-textTertiary" />
            ...
            {filteredAndSortedItems.map((item) => (
              <li
                ...
                className={classNames(
                  'text-flowstarter-elements-textTertiary flex items-center gap-2 px-2 py-1 rounded hover:bg-flowstarter-elements-background-depth-2 transition-colors group',
                  selectedItems.has(item.path) ? 'bg-flowstarter-elements-background-depth-2' : '',
                )}
              >
                <Checkbox
                  checked={selectedItems.has(item.path)}
                  onCheckedChange={() => handleSelectItem(item.path)}
                  className="w-3 h-3 rounded border-flowstarter-elements-borderColor"
                  aria-labelledby={`item-label-${item.path}`} // For accessibility
                />
                <span
                  className={classNames(
                    'shrink-0 text-flowstarter-elements-textTertiary text-xs',
                ...
                ></span>
                <button
                  className="flex items-center px-1 py-0.5 text-xs rounded bg-transparent hover:bg-flowstarter-elements-background-depth-3"
                  onClick={() => {
                    if (item.type === 'file') {
      ...
      {/* Footer */}
      <div className="px-2 py-1 border-t border-flowstarter-elements-borderColor bg-flowstarter-elements-background-depth-2 text-xs text-flowstarter-elements-textTertiary flex justify-between items-center">
        <div>
