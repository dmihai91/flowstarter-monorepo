...
export async function streamText(props: {
  ...
  let processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = content.replace(/<div class=\\"__FlowstarterThought__\\">.*?<\/div>/s, '');
      content = content.replace(/<div class=\\"__FlowstarterThinking__\\"[^>]*>.*?<\/div>/gs, '');
      content = content.replace(/<think>.*?<\/think>/s, '');
      content = content.replace(/<FlowstarterThinking[^>]*>.*?<\/FlowstarterThinking>/gs, '');

      content = content.replace(
        /<FlowstarterAction type="file" filePath="package-lock\.json">[\s\S]*?<\/FlowstarterAction>/g,
