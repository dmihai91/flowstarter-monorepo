...
export function simplifyFlowstarterActions(input: string): string {
  // Using regex to match FlowstarterAction tags that have type="file"
  const regex = /(<FlowstarterAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/FlowstarterAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<FlowstarterAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</FlowstarterAction>`;
    });

  return `<FlowstarterArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</FlowstarterArtifact>`;
}
...
export function createReferencedFilesContext(
  ...
  const fileContexts = referencedPaths
    .map((relativePath) => {
      ...
      return `<FlowstarterAction type="file" filePath="${relativePath}">${dirent.content}</FlowstarterAction>`;
    })
    .filter((ctx) => ctx !== '');
  ...
  return `<FlowstarterArtifact id="referenced-files" title="Referenced Files">\n${fileContexts.join('\n')}\n</FlowstarterArtifact>`;
}
