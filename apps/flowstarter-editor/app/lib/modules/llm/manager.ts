...
export class LLMManager {
  ...
  private async _registerProvidersFromDirectory() {
    try {
      ...
      logger.debug('Flowstarter: Initializing AI provider registration...');

      const exportedItems = Object.values(providers);
      logger.debug(`Flowstarter: Discovered ${exportedItems.length} provider modules in registry`);
      ...
      logger.info(`Flowstarter: Successfully registered ${registeredCount} AI providers`);

      if (registeredCount === 0) {
        logger.error('Flowstarter WARNING: No AI providers were registered! Model list will be empty.');
      }
    } catch (error) {
  ...
  registerProvider(provider: BaseProvider) {
    ...
    logger.info('Flowstarter: Activating AI Provider -', provider.name);
  ...
  async updateModelList(options: {
    ...
    logger.debug('Flowstarter: Updating AI model catalog', {
    ...
    if (this._providers.size === 0) {
      logger.warn('Flowstarter: No AI providers available! Model catalog will be empty.');
      return [];
    }
    ...
    logger.debug(`Flowstarter: ${staticModels.length} base AI models loaded`);
      ...
      Array.from(this._providers.values())
        .filter((provider) => {
          if (!enabledProviders.includes(provider.name)) {
            logger.debug(`Flowstarter: Skipping ${provider.name} (disabled in settings)`);
            return false;
          }
        ...
        .map(async (provider) => {
          ...
          if (cachedModels) {
            logger.debug(`Flowstarter: Using ${cachedModels.length} cached models from ${provider.name}`);
            return cachedModels;
          }
          ...
          if (!hasRequiredConfig) {
            logger.debug(`Flowstarter: Skipping ${provider.name} (missing API configuration)`);
            return [];
          }
          ...
          const dynamicModels = await provider
            ...
            .then((models) => {
              logger.info(`Flowstarter: Cached ${models.length} AI models from ${provider.name}`);
            ...
            .catch((err) => {
              ...
              if (providerStaticModels.length > 0) {
                logger.info(`Flowstarter: ${provider.name} - Using ${providerStaticModels.length} fallback models`);
              }
    ...
    logger.debug(
      `Flowstarter: Retrieved ${dynamicModelsFlat.length} dynamic + ${filteredStaticModels.length} static AI models`,
    );
    ...
    logger.info(`Flowstarter: AI model catalog ready with ${modelList.length} total models`);
