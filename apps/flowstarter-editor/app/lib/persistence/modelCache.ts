...
const STORAGE_KEY = 'Flowstarter_model_cache';
...
export function saveModelsToCache(provider: string, cacheId: string, models: ModelInfo[]): void {
  ...
  try {
    ...
    logger.debug(`Flowstarter: Persisted ${models.length} models for ${provider} to localStorage`);
  } catch (error) {
    logger.error(`Flowstarter: Failed to save models for ${provider} to localStorage`, error);
  }
}
...
export function loadModelsFromCache(provider: string, cacheId: string): ModelInfo[] | null {
  ...
  try {
    ...
    if (cached.version !== CACHE_VERSION) {
      logger.debug(`Flowstarter: Cache version mismatch for ${provider}, invalidating`);
    ...
    if (age > CACHE_TTL_MS) {
      logger.debug(`Flowstarter: Cache expired for ${provider} (age: ${Math.round(age / 1000 / 60)} minutes)`);
      delete storage.providers[provider];
      localStorage.setItem(STORAGE_KEY, JSON.stringify(storage));
    ...
    if (cached.cacheId !== cacheId) {
      logger.debug(`Flowstarter: Cache ID mismatch for ${provider}, invalidating`);
      delete storage.providers[provider];
      localStorage.setItem(STORAGE_KEY, JSON.stringify(storage));
    ...
    logger.debug(`Flowstarter: Loaded ${cached.models.length} models for ${provider} from localStorage`);

    return cached.models;
  } catch (error) {
    logger.error(`Flowstarter: Failed to load models for ${provider} from localStorage`, error);
    return null;
  }
}
...
export function clearProviderCache(provider: string): void {
  ...
  try {
    ...
    delete storage.providers[provider];
    localStorage.setItem(STORAGE_KEY, JSON.stringify(storage));
    logger.info(`Flowstarter: Cleared cache for ${provider}`);
  } catch (error) {
    logger.error(`Flowstarter: Failed to clear cache for ${provider}`, error);
  }
}
...
export function clearAllModelsCache(): void {
  ...
  try {
    ...
    logger.info('Flowstarter: Cleared all model caches from localStorage');
  } catch (error) {
    logger.error('Flowstarter: Failed to clear all model caches', error);
  }
}
...
export function getCacheStats(): {
  ...
  try {
    ...
    logger.error('Flowstarter: Failed to get cache stats', error);
    return { totalProviders: 0, totalModels: 0, providers: [] };
  }
}
...
function loadCacheFromStorage(): ModelCacheStorage {
  try {
    ...
    if (!parsed.version || !parsed.providers || typeof parsed.providers !== 'object') {
      logger.warn('Flowstarter: Invalid cache structure, resetting');
      return {
        version: CACHE_VERSION,
        providers: {},
      };
    ...
    logger.error('Flowstarter: Failed to load cache from localStorage, resetting', error);
    return {
      version: CACHE_VERSION,
      providers: {},
    };
