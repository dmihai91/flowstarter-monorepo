...
import type {
  ...
  FileAction,
  ShellAction,
  FlowstarterArtifactData,
  ThinkingArtifactData,
  ThinkingData,
  MessageState,
...
import {
  ...
  FLOWSTARTER_ARTIFACT_TAG_OPEN,
  FLOWSTARTER_ARTIFACT_TAG_CLOSE,
  FLOWSTARTER_ACTION_TAG_OPEN,
  FLOWSTARTER_ACTION_TAG_CLOSE,
  THINKING_TAG_OPEN,
  THINKING_TAG_CLOSE,
  THINKING_ARTIFACT_TAG_OPEN,
  THINKING_ARTIFACT_TAG_CLOSE,
  MAX_FILE_CHUNK_SIZE,
  MAX_TAG_LENGTH,
  createArtifactElement,
  createThinkingArtifactElement,
...
export const extractAttribute = (tag: string, name: string) => tag.match(new RegExp(`${name}="([^"]*)"`, 'i'))?.[1];
...
export function handleActionContent(
  input: string,
  state: MessageState,
  opts: StreamingMessageParserOptions,
  messageId: string,
  i: number,
): ParseResult {
  ...
  if (closeIdx === -1) {
    closeIdx = input.indexOf(FLOWSTARTER_ACTION_TAG_CLOSE, i);
  }
  ...
  if (closeIdx !== -1) {
    ...
    const len =
      input.indexOf(ARTIFACT_ACTION_TAG_CLOSE, i) === closeIdx
...
export function handleArtifactContent(
  ...
  if (fsActionIdx !== -1 && (actionIdx === -1 || fsActionIdx < actionIdx)) {
    actionIdx = fsActionIdx;
  }

  if (fsCloseIdx !== -1 && (closeIdx === -1 || fsCloseIdx < closeIdx)) {
    closeIdx = fsCloseIdx;
  }

  if (thinkCloseIdx !== -1 && (closeIdx === -1 || thinkCloseIdx < closeIdx)) {
  ...
  if (closeIdx !== -1) {
    ...
    const len =
      input.indexOf(ARTIFACT_TAG_CLOSE, i) === closeIdx
...
function openTag(
  ...
  if (end === -1) {
    return { output: '', position: i, done: true, earlyBreak: true };
  }
  ...
  if (state.insideThinkingArtifact) {
    ...
    if (taCloseIdx !== -1) {
      if (state.currentThinkingArtifact) {
        state.currentThinkingArtifact.content += input.slice(i, taCloseIdx);
      }
