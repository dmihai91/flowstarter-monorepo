...
const INSTALLED_PACKAGES = new Set(['astro', '@astrojs/tailwind', 'tailwindcss']);
...
function isValidPackage(source: string): boolean {
  ...
  // Check exact match
  if (INSTALLED_PACKAGES.has(source)) {
    return true;
  }
...
function getKnownBadReason(source: string): string | undefined {
  // Direct match
  if (KNOWN_BAD_PACKAGES[source]) {
    return KNOWN_BAD_PACKAGES[source];
  }
...
function removeImportAndUsages(content: string, imp: ImportInfo): string {
  ...
  if (namedImportMatch) {
    ...
    for (const name of names) {
      fixed = fixed.replace(new RegExp(`<${name}\\s+[^>]*\\/?>`, 'g'), '<!-- removed: ' + name + ' -->');
      fixed = fixed.replace(
 ...
 */
function checkUndefinedComponents(filePath: string, content: string, imports: ImportInfo[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  if (parts.length < 3) {
    return issues;
  } // No frontmatter or not an .astro file
  ...
  for (const imp of imports) {
    ...
    if (combinedMatch) {
      importedNames.add(combinedMatch[1]);
    }
  }
 ...
 */
function checkFrontmatterSyntax(filePath: string, content: string): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
function fixUntypedArrays(content: string): string {
   ...
   */
  fm = fm.replace(/^(\s*const\s+\w+)\s*=\s*\[/gm, (match, prefix) => {
...
function fixStringLiteralProps(content: string): string {
  return content.replace(/interface Props \{([^}]*)\}/gs, (_match: string, body: string) => {
    const fixedBody = body.replace(/: "[^"]*"/g, ': string').replace(/: '[^']*'/g, ': string');
...
export function validateAndFixFiles(files: Record<string, string>): ValidationResult {
  const issues: ValidationIssue[] = [];
  ...
  for (const [filePath, content] of Object.entries(files)) {
    ...
    if (!/\.(astro|ts|tsx|js|jsx)$/.test(filePath)) {
      continue;
    }
    ...
    for (const imp of imports) {
      if (isLocalImport(imp.source)) {
        continue;
      } // Skip local imports for package validation
