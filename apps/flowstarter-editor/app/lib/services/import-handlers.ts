...
function importEntries(entries: Record<string, unknown>, setItem: SetItemFn, label: string): void {
...
function importProviderSettings(providers: Record<string, unknown>, setItem: SetItemFn, setCookie: SetCookieFn): void {
  if (providers.provider_settings) {
    try {
      setItem('provider_settings', providers.provider_settings);
    } catch (err) {
...
function importUISettings(ui: Record<string, unknown>, setItem: SetItemFn, setCookie: SetCookieFn): void {
  if (ui.Flowstarter_tab_configuration) {
    try {
      setItem('Flowstarter_tab_configuration', ui.Flowstarter_tab_configuration);
    } catch (err) {
...
function importConnectionSettings(connections: Record<string, unknown>, setItem: SetItemFn): void {
  if (connections.netlify_connection) {
    try {
      setItem('netlify_connection', connections.netlify_connection);
    } catch (err) {
...
function importDebugSettings(debug: Record<string, unknown>, setItem: SetItemFn, setCookie: SetCookieFn): void {
...
function importUpdateSettings(updates: Record<string, unknown>, setItem: SetItemFn): void {
  if (updates.update_settings) {
    try {
      setItem('update_settings', updates.update_settings);
    } catch (err) {
  ...
  if (updates.last_acknowledged_update) {
    try {
      setItem('Flowstarter_last_acknowledged_version', updates.last_acknowledged_update);
    } catch (err) {
