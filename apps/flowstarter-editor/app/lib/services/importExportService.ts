...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          Flowstarter_user_profile: this._safeGetItem('Flowstarter_user_profile'),
          Flowstarter_settings: this._safeGetItem('Flowstarter_settings'),
          Flowstarter_profile: this._safeGetItem('Flowstarter_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('Flowstarter_viewed_features'),
          developer_mode: this._safeGetItem('Flowstarter_developer_mode'),
        ...
        ui: {
          // Tab configuration
          Flowstarter_tab_configuration: this._safeGetItem('Flowstarter_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('Flowstarter_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('Flowstarter_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          Flowstarter_read_logs: this._safeGetItem('Flowstarter_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('Flowstarter_last_acknowledged_version'),
        },
  ...
  private static async _importComprehensiveFormat(data: Partial<ExportedSettings>): Promise<void> {
    ...
    if (data.providers) {
      ...
      providerCookies.forEach((key) => {
        if (providers && providers[key]) {
          try {
            this._safeSetCookie(key, providers[key]);
          } catch (err) {
    ...
    if (data.ui) {
      ...
      if (data.ui.Flowstarter_tab_configuration) {
        try {
          this._safeSetItem('Flowstarter_tab_configuration', data.ui.Flowstarter_tab_configuration);
        } catch (err) {
      ...
      uiCookies.forEach((key) => {
        if (ui && ui[key]) {
          try {
            this._safeSetCookie(key, ui[key]);
          } catch (err) {
    ...
    if (data.debug) {
      ...
      debugLocalStorageKeys.forEach((key) => {
        if (debug[key] !== null && debug[key] !== undefined) {
          try {
            this._safeSetItem(key, debug[key]);
          } catch (err) {
      ...
      debugCookies.forEach((key) => {
        if (debug[key]) {
          try {
            this._safeSetCookie(key, debug[key]);
          } catch (err) {
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('Flowstarter_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
