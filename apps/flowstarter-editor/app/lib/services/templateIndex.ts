...
export function buildTemplateIndex(files: GeneratedFile[]): string {
  ...
  const sections = indexed.map((f) => `### ${f.path} (${f.lines} lines)\n${f.structure}`);
...
function indexFile(file: GeneratedFile): FileIndex {
  ...
  if (file.path.endsWith('.mjs') && file.path.includes('tailwind')) {
    ...
    if (colorMatch) {
      structure.push(`Colors: ${colorMatch[1].trim().slice(0, 200)}`);
    }

    const fontMatch = file.content.match(/fontFamily:\s*\{([^}]+)\}/s);

    if (fontMatch) {
      structure.push(`Fonts: ${fontMatch[1].trim().slice(0, 200)}`);
    }
    ...
    if (fmMatch) {
      ...
      if (components.length) {
        structure.push(`Components used: ${components.join(', ')}`);
      }
    }
    ...
    if (sectionHeaders.length) {
      structure.push(`Sections: ${sectionHeaders.join(' | ')}`);
    }
    ...
    if (slots) {
      structure.push(`Slots: ${[...new Set(slots)].join(', ')}`);
    }
