...
class LogStore {
  ...
  private _logsLoaded = false;
  private _readLogsLoaded = false;

  constructor() {
    /*
     * OPTIMIZATION: Defer loading logs until first access
     * This improves startup time by not parsing cookies/localStorage immediately
     */
  }

  // Lazy load logs on first access
  private _ensureLogsLoaded() {
    if (this._logsLoaded) {
      return;
    }
  ...
  // Lazy load read logs on first access
  private _ensureReadLogsLoaded() {
