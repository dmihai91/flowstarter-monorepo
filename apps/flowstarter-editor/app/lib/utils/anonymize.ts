...
const PII_PATTERNS = [
  ...
  {
    pattern:
      /\b\d{1,5}\s+[A-Za-z]+\s+(Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Way|Court|Ct|Place|Pl|Circle|Cir)\b\.?/gi,
    replacement: '[ADDRESS]',
  },
...
const NAME_PATTERNS = [
  ...
  {
    pattern: /\b(my name is|i'm|i am|call me|this is|name:)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b/gi,
    replacement: '$1 [NAME]',
  },
  ...
  {
    pattern: /\b(contact|reach|email|call)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s+(at|on)\b/gi,
    replacement: '$1 [NAME] $3',
  },

  // Signatures: "Thanks, John" / "Best, John Smith"
  {
    pattern: /\b(thanks|thank you|regards|best|sincerely|cheers),?\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s*$/gim,
    replacement: '$1, [NAME]',
  },
  ...
  {
    pattern:
      /\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)'s\s+(business|company|shop|store|studio|clinic|practice|website|site|brand)/gi,
    replacement: "[OWNER]'s $2",
  },
  ...
  {
    pattern: /\b(for|about)\s+([A-Z][a-zA-Z\s&]{2,30})\s+(business|company|brand|shop|studio|clinic)\b/gi,
    replacement: '$1 [BUSINESS] $3',
  },
];
...
export function anonymizeQuery(query: string, maxLength: number = 500): string {
  ...
  result = result.replace(
    /\b([A-Z][a-z]{1,14})\s+(is|was|has|had|will|would|can|could|wants?|needs?|said|asked)\b/g,
    '[NAME] $2',
  );
