...
const MAIN_PLATFORM_URL =
  typeof window !== 'undefined'
    ? (window as any).__ENV__?.MAIN_PLATFORM_URL ||
...
export default function MagicLinkComplete() {
  return <ClientOnly fallback={<LoadingState />}>{() => <MagicLinkCompleteContent />}</ClientOnly>;
}
  ...
  return (
    ...
    >
      <div style={{ textAlign: 'center' }}>
        ...
        />
        <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.8)' }}>Setting up your account...</p>
        <style>{`
  ...
  useEffect(() => {
    async function complete() {
      ...
      try {
        ...
        if (handoffToken) {
          ...
          if (res.ok) {
            const data = (await res.json()) as {
              userId?: string;
              user?: { signupMethod?: 'google' | 'apple' | 'email' };
            };
        ...
        if (!clerkUserId) {
          /* Try to get from session/cookie. The main platform should have set this. */
        ...
        localStorage.setItem('flowstarter_project_url_id', result.projectUrlId!);
        localStorage.setItem('flowstarter_client_session_token', result.sessionToken!);
    ...
    return (
      ...
      >
        <div style={{ textAlign: 'center', maxWidth: '400px', padding: '32px' }}>
          ...
          <h1 style={{ fontSize: '24px', fontWeight: 600, marginBottom: '12px' }}>Setup Failed</h1>
          <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.7)', marginBottom: '24px' }}>{error}</p>
          <button
