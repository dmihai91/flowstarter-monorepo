...
function getHandoffSecret(): string | null {
  return process.env.HANDOFF_SECRET || process.env.VITE_HANDOFF_SECRET || null;
}
...
function verifyToken(token: string): HandoffPayload | null {
  const handoffSecret = getHandoffSecret();
  ...
  try {
    ...
    const expectedSig = createHmac('sha256', handoffSecret).update(data).digest('base64url');
    ...
    const payload = JSON.parse(Buffer.from(data, 'base64url').toString('utf8')) as HandoffPayload;
...
export async function loader({ request }: LoaderFunctionArgs) {
  if (!getHandoffSecret()) {
    return json({ valid: false, error: 'HANDOFF_SECRET is not configured' }, { status: 500 });
  }
