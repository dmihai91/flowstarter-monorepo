...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .flowstarter
  filteredFiles = filteredFiles.filter((x: { path: string }) => x.path.startsWith('.flowstarter') == false);

  // check for ignore file in .flowstarter folder
  const templateIgnoreFile = files.find(
    (x: { path: string; name: string }) => x.path.startsWith('.flowstarter') && x.name == 'ignore',
  );
...
Flowstarter is initializing your project with the required files using the ${template.name} template.
<flowstarterArtifact id="imported-files" title="${title || 'Create initial files'}" type="bundled">
${filesToImport.files
  .map(
    (file: { path: string; content: string }) =>
      `<flowstarterAction type="file" filePath="${file.path}">
${file.content}
</flowstarterAction>`,
  )
  .join('\n')}
...
`;
  ...
  const templatePromptFile = files
    .filter((x: { path: string }) => x.path.startsWith('.flowstarter'))
