 ...
 */
function getCachedPreviewUrl(projectId) {
...
async function getSandboxInfo(s) {
    return s.info();
}
/**
 ...
 */
async function findReusableSandbox(client, projectId) {
    try {
        const sandboxes = await client.list();
        // Get info for all sandboxes to check labels and state
        const sandboxInfos = await Promise.all(sandboxes.map(async (s) => ({ sandbox: s, info: await getSandboxInfo(s) })));
        ...
        const anyAvailable = sandboxInfos.find((si) => !si.info.labels?.project &&
 ...
 */
async function createSandbox(client, projectId) {
    const baseConfig = {
        envVars: {
    ...
    for (const bunImage of bunImages) {
        try {
            const sandbox = await client.create({ ...baseConfig, image: bunImage }, 120);
            return sandbox;
        }
        catch {
            // Try next image
    ...
    // Fallback to node
    const sandbox = await client.create({ ...baseConfig, language: 'javascript' }, 120);
    return sandbox;
}
/**
 * Ensure sandbox is running, restart if needed.
 */
async function ensureSandboxRunning(sandbox) {
    try {
 ...
 */
async function getPreviewUrl(sandbox) {
 ...
 */
async function getOrCreateSandbox(client, projectId) {
...
export { clearSandboxCache, createSandbox, deleteCachedSandbox, ensureSandboxRunning, findReusableSandbox, getCachedPreviewUrl, getCachedSandbox, getClient, getOrCreateSandbox, getPreviewUrl, setCachedSandbox };
