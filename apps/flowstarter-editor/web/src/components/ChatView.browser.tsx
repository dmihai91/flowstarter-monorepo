...
import { DEFAULT_CLIENT_SETTINGS } from "@flowstarter/editor-contracts/settings";
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("flowstarter-editor:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    localStorage.setItem(
      "flowstarter-editor:client-settings:v1",
      JSON.stringify({
        ...DEFAULT_CLIENT_SETTINGS,
    ...
    try {
      ...
      localStorage.removeItem("flowstarter-editor:client-settings:v1");
      await mounted.cleanup();
    }
  });
