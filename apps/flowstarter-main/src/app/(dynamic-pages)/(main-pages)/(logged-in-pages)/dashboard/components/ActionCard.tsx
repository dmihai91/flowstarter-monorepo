...
export function ActionCard({
  ...
  const inner = (
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        ...
        {elevated && (
          <p className="text-xs text-white/30 mt-1">First step to getting your site live</p>
        )}
      </div>

      <ChevronRight className={cn(
        'w-4 h-4 flex-shrink-0 transition-all duration-300 group-hover:translate-x-1',
        elevated ? 'text-[var(--purple)]' : 'text-white/30 group-hover:text-white/60',
      )} />
    </div>
  );
  ...
  if (elevated) {
    return (
      <div className={cn('p-[1px] rounded-xl bg-gradient-to-r from-[var(--purple)] via-blue-500 to-cyan-500 shadow-[0_4px_24px_rgba(77,93,217,0.25)]', className)}>
        <Tag {...interactiveProps} className="block rounded-[calc(12px-1px)] overflow-hidden">
          {inner}
        </Tag>
      </div>
    );
