  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className={cn(
        ...
        status === 'active'    ? 'text-[var(--purple)]'                        :
        status === 'completed' ? 'text-emerald-500 dark:text-emerald-400'      :
...
function StepCard({ index, title, desc, status, showDesc = false }: {
  index: number; title: string; desc: string; status: StepStatus; showDesc?: boolean;
}) {
  return (
    <div className={cn(
      ...
      <h3 className={cn(
        ...
        status === 'locked' ? 'text-gray-400 dark:text-white/30' : 'text-gray-900 dark:text-white'
      )}>
        {title}
      </h3>
      {showDesc && (
        <p className={cn(
          ...
          status === 'locked' ? 'text-gray-300 dark:text-white/20' : 'text-gray-500 dark:text-white/50'
        )}>
          {desc}
        </p>
      )}
...
export function MilestonesTimeline({ hasAnyProject, hasLiveProject }: MilestonesTimelineProps) {
  ...
  const milestones = [
    { title: t('dashboard.stepper.strategy'),    desc: t('dashboard.stepper.strategyDescription'),    status: statuses[0] as StepStatus },
    { title: t('dashboard.stepper.design'),      desc: t('dashboard.stepper.designDescription'),      status: statuses[1] as StepStatus },
    { title: t('dashboard.stepper.development'), desc: t('dashboard.stepper.developmentDescription'), status: statuses[2] as StepStatus },
    { title: t('dashboard.stepper.launch'),      desc: t('dashboard.stepper.launchDescription'),      status: statuses[3] as StepStatus },
  ];
          ...
          <div className="space-y-3">
            {milestones.map((m, i) => (
              <div key={i} className="relative flex items-center gap-3">
                ...
                <StepCard
                  index={i} title={m.title} desc={m.desc}
                  status={m.status} showDesc={m.status === 'active'}
                />
              </div>
            ))}
          </div>
          ...
          <div key={i} className={cn(
            ...
            <div className="flex-1 min-w-0">
              ...
              <h3 className={cn(
                ...
                m.status === 'locked' ? 'text-gray-400 dark:text-white/30' : 'text-gray-900 dark:text-white'
              )}>
                {m.title}
              </h3>
              {m.status === 'active' && (
                <p className="text-xs text-gray-500 dark:text-white/50 leading-snug mt-0.5">{m.desc}</p>
              )}
            </div>
          </div>
        ))}
