    ...
    <PageContainer gradientVariant="help">
      <div className="space-y-12">
        ...
        <section ref={stepsRef}>
          <h2 className={`text-xl font-semibold text-gray-900 dark:text-white mb-6 transition-all duration-500 ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {steps.map((step, index) => {
              ...
              const animation = getStaggeredAnimation(index, stepsVisible);
              return (
                <div key={step.number} className={animation.className} style={animation.style}>
                  <GlassCard className="h-full">
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center gap-3">
                        <div 
                          className="w-10 h-10 rounded-xl flex items-center justify-center"
                          style={{ backgroundColor: `color-mix(in srgb, ${step.iconColor} 10%, transparent)` }}
                        >
                          <IconComponent 
                            className="w-5 h-5" 
        ...
        {/* CTA Card */}
        <section className={`transition-all duration-500 ease-out delay-200 ${
          stepsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
        }`}>
          <GlassCard variant="elevated">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-[#1a1a2e] via-[#16213e] to-[#1a1a2e] dark:from-white dark:via-gray-100 dark:to-white text-white dark:text-gray-900 font-semibold shadow-lg hover:shadow-[0_0_20px_rgba(77,93,217,0.2)] transition-all duration-300 hover:scale-[1.02]"
              >
        ...
        <section ref={faqRef}>
          ...
          >
            {faqs.map((faq, index) => {
              const animation = getStaggeredAnimation(index, faqVisible);
              return (
                <div key={`faq-${index}`} className={animation.className} style={animation.style}>
                  <AccordionItem
                    value={`item-${index}`}
                    className="border border-gray-200/60 dark:border-white/10 rounded-2xl px-6 bg-white/70 dark:bg-white/[0.04] backdrop-blur-sm"
                  >
