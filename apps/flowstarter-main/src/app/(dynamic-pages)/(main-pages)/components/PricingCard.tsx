...
export function PricingCard({
  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl p-8 transition-all duration-300 backdrop-blur-xl ${
        recommended
    ...
    >
      ...
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-[var(--fs-ink)]">{name}</h3>
      ...
      <ul className="mb-8 flex-1 space-y-3">
        {features.map((feature) => (
          ...
          >
            <svg
              className="mt-0.5 h-4 w-4 flex-shrink-0 text-[var(--landing-text-accent)]"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={2.5}
              stroke="currentColor"
