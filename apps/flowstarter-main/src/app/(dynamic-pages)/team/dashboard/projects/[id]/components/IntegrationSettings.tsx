    ...
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Your public Calendly scheduling page
            </p>
          </div>
          ...
          <div>
            <Label htmlFor="calendlyApiKey">
              ...
              <span className="text-gray-400">
                (optional, for service-specific buttons)
              </span>
            </Label>
          ...
          <div className="flex gap-2 pt-2">
            <Button
              onClick={() =>
                saveIntegration('calendly', {
                  calendlyUrl,
                  calendlyApiKey: calendlyApiKey || undefined,
                })
              }
            ...
            {status?.calendly.url && (
              <Button
                ...
                size="sm"
                onClick={() =>
                  saveIntegration('calendly', { action: 'disconnect' })
                }
      ...
      <div className={cardClass}>
        ...
        <div className="space-y-3">
          <div>
            ...
            />
            <p className="text-xs text-gray-400 mt-1">
              Numeric property ID from Google Analytics admin
            </p>
          </div>
          ...
          <div className="flex gap-2 pt-2">
            {!status?.analytics.connected ? (
              <Button
                onClick={() => {
                  if (gaPropertyId)
                    saveIntegration('analytics', { gaPropertyId });
                  window.open(
                    `/api/analytics/connect?projectId=${projectId}`,
                    '_blank'
                  );
                }}
                size="sm"
              ...
              <>
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    saveIntegration('analytics', { action: 'disconnect' })
                  }
                  className="text-red-500 hover:text-red-600"
                >
                  <Trash2 className="w-3.5 h-3.5 mr-1" /> Disconnect
          ...
          {status?.analytics.connected && status.analytics.connectedAt && (
            <p className="text-xs text-gray-400">
              Connected{' '}
              {new Date(status.analytics.connectedAt).toLocaleDateString()}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-start gap-2 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200/50 dark:border-emerald-700/20">
        ...
        <p className="text-xs text-emerald-700 dark:text-emerald-300">
          All API keys and tokens are encrypted at rest using Supabase Vault
          (pgsodium). Only encrypted references are stored in the database.
        </p>
      </div>
    </div>
  );
