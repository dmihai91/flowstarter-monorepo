...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const { data: project, error } = await supabase
    .from('projects')
    .select('id, name, user_id, deposit_amount, final_amount, stripe_customer_id')
    .eq('id', projectId)
    .single();
  ...
  const clientName  = [clerkUser.firstName, clerkUser.lastName].filter(Boolean).join(' ') || 'Client';

  if (!clientEmail) return NextResponse.json({ error: 'Client has no email in Clerk' }, { status: 422 });
  ...
  if (!stripeCustomerId) {
    ...
    await supabase.from('projects').update({ stripe_customer_id: stripeCustomerId }).eq('id', projectId);
  }
