...
export async function GET() {
  ...
  const { data: projects, error } = await supabase
    .from('projects')
    .select('id, name, status, user_id, setup_fee, created_at, updated_at')
    .not('status', 'eq', 'prefill')
  ...
  for (const p of projects ?? []) {
    ...
    if (ex) {
      ex.projectCount++;
      ex.totalFee += Number(p.setup_fee ?? 0);
      ex.statuses.push(p.status ?? 'draft');
      if (ua > ex.lastActivity) ex.lastActivity = ua;
    } else
      grouped.set(uid, {
        userId: uid,
        projectCount: 1,
        totalFee: Number(p.setup_fee ?? 0),
        statuses: [p.status ?? 'draft'],
        lastActivity: ua,
      });
  }
