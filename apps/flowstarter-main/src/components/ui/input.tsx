...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'file:text-foreground placeholder:text-muted-foreground flex h-8 w-full min-w-0 rounded-xl border px-2.5 py-1 text-sm shadow-sm transition-[color,box-shadow,border-color,background-color] outline-none file:inline-flex file:h-6 file:border-0 file:bg-transparent file:text-xs file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 text-gray-900 dark:text-gray-100',
        // Light mode - solid white
        'bg-white border-gray-300/90 hover:border-gray-400 focus:border-purple-600/70 focus-visible:border-purple-600/50 focus-visible:ring-1 focus-visible:ring-purple-500/20',
