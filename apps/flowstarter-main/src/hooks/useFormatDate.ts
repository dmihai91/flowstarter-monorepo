...
export function useFormatDate(defaultLocale?: string) {
  const locale = defaultLocale || (typeof navigator !== 'undefined' ? navigator.language : 'en-US');
   ...
   */
  const formatRelative = useCallback((date: Date | string | null): string => {
    if (!date) return '';
   ...
   */
  const formatTimeAgo = useCallback((date: Date | string | null): string => {
    if (!date) return '';
