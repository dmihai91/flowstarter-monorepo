...
function loadKey(): void {
  ...
  for (const f of ['.env.shared', 'apps/flowstarter-main/.env', 'apps/flowstarter-main/.env.local', 'apps/flowstarter-editor/server/.env']) {
    ...
    if (m?.[1]) { process.env.OPENROUTER_API_KEY = m[1].trim(); return; }
  }
}

async function main() {
  loadKey();
  if (!process.env.OPENROUTER_API_KEY) { console.error('✗ no OPENROUTER_API_KEY'); process.exit(2); }

  const [wall] = process.argv.slice(2);
  console.log(`▶ personalize → "${sampleSpec.businessName}" (${sampleSpec.industry})`);
  console.log(`  Sonnet plans+critiques · Kimi K2.6 implements · target≤${TARGET_MS / 1000}s\n`);
  const t0 = Date.now();
  let last = '';
  ...
  console.log(`cost:            $${r.costUsd.toFixed(4)}   implementer passes: ${r.attempts}`);
